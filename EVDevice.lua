...
function EVDevice.isLikeFlightStick(self)
  return self:hasEventCode(input.Constants.EV_ABS, input.Constants.ABS_X)
    and self:hasEventCode(input.Constants.EV_ABS, input.Constants.ABS_Y)
    and self:hasEventCode(input.Constants.EV_ABS, input.Constants.ABS_THROTTLE)
end
...
function EVDevice.isLikeMouse(self)
  if
    self:hasEventType(input.Constants.EV_REL)
    and self:hasEventCode(input.Constants.EV_REL, input.Constants.REL_X)
    and self:hasEventCode(input.Constants.EV_REL, input.Constants.REL_Y)
    and self:hasEventCode(input.Constants.EV_KEY, input.Constants.BTN_LEFT)
  then
    return true
  end
...
function EVDevice.isLikeTablet(self)
  if
    self:hasEventType(input.Constants.EV_ABS) and self:hasEventCode(input.Constants.EV_KEY, input.Constants.BTN_LEFT)
  then
    return true
  end
...
function EVDevice.events(self, predicate)
  local function iter_gen(params, flags)
    ...
    if params.Predicate then
      ...
      until rc ~= ffi.C.LIBEVDEV_READ_STATUS_SUCCESS and rc ~= LIBEVDEV_READ_STATUS_SYNC and rc ~= -libc.EAGAIN
    else
      repeat
        rc = evdev.libevdev_next_event(params.Handle, flags, ev)
